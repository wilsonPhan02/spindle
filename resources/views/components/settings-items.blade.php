...
@elseif($variant === 'dropdown')
    ...
    >
        ...
        <button
            type="button"
            @click="open = !open"
            class="w-full flex items-center justify-between py-3 px-4 -mx-4 transition-colors rounded-xl group hover:bg-black/5 focus:outline-none"
        >
            ...
            <div class="flex items-center gap-2">
                @if($activeOption)
                    <div class="flex items-center gap-2 px-2.5 py-1 rounded-full bg-black/5 border border-black/10 group-hover:bg-black/10 transition-colors">
                        {{-- Bendera lingkaran --}}
                        <div class="w-5 h-5 rounded-full overflow-hidden border border-black/10 shrink-0">
                            <img
        ...
        <div
            ...
            class="absolute right-0 top-full mt-1 w-64 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden"
        >
            {{-- Search input --}}
            <div class="p-2 border-b border-gray-200">
                <div class="relative">
                    ...
                    <input
                        ...
                        @click.stop
                        class="w-full pl-8 pr-3 py-1.5 text-[13px] bg-white border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-secondary-150 text-gray-700 placeholder-gray-400"
                        x-ref="searchInput"
                        x-init="$watch('open', v => {
                            if (v) {
                                $nextTick(() => {
                                    $refs.searchInput.focus();
                                    const el = $refs.list.querySelector('.bg-gray-50');
            ...
            <div class="overflow-y-auto max-h-52 py-1 custom-scrollbar" x-ref="list">
                <template x-for="option in filtered" :key="option.code">
                    <button
                        ...
                        class="w-full flex items-center gap-3 px-3 py-2 hover:bg-gray-50 transition-colors text-left group"
                        :class="option.code === {{ Illuminate\Support\Js::from($current) }} ? 'bg-gray-50' : ''"
                    >
                        {{-- Bendera bulat --}}
                        <div class="w-7 h-7 rounded-full border border-gray-200 overflow-hidden shrink-0">
                            <img
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-[11px] text-gray-500 uppercase tracking-wider" x-text="option.code"></div>
                        </div>
                        {{-- Active checkmark --}}
                        <svg
...
@else
    ...
    >
        ...
        @if($variant === 'toggle')
            <div
                class="w-11 h-6 rounded-full relative shadow-inner transition-colors duration-300 border border-black/5"
                :class="isOn ? 'bg-secondary-300 border-secondary-300' : 'bg-black/5 group-hover:bg-black/10'"
            >
                <div
